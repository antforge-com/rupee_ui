...
const AddAdvisor: React.FC<AddAdvisorProps> = ({ onClose, onSave }) => {
  const [formData, setFormData] = useState({
    ...
    shiftEndTime: "",
    charges: "",
    displayPrice: "",  // ADDED: customer-facing marked-up price
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const consultantPayload = {
        ...
        displayPrice:   formData.displayPrice ? parseFloat(formData.displayPrice) : undefined, // ADDED: customer-facing price
    ...
    <div className="advisor-modal-overlay">
      <div className="advisor-card" style={{ maxWidth: 680, width: "100%" }}>
        ...
        <form className="advisor-form" onSubmit={handleSubmit}>
          ...
          {/* ── Availability Times — RENAMED from "Shift Times" as per requirements ── */}
          <div style={{ display: "flex", gap: 12 }}>
            <div className="form-group" style={{ flex: 1 }}>
              <label>AVAILABILITY START <span className="required">*</span></label>
              <div
            ...
            <div className="form-group" style={{ flex: 1 }}>
              <label>AVAILABILITY END <span className="required">*</span></label>
              <div
          ...
          {/* ── Charge — UPDATED: now shows base + display price ── */}
          <div style={{ display: "flex", gap: 12 }}>
            <div className="form-group" style={{ flex: 1 }}>
              <label>BASE CHARGE PER PERSON (₹) <span className="required">*</span></label>
              <input type="number" name="charges" placeholder="e.g. 200"
                value={formData.charges} onChange={handleChange}
                disabled={loading} min="0" step="0.01" />
              <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 4 }}>
                Internal base cost (e.g. ₹200)
              </div>
            </div>
            <div className="form-group" style={{ flex: 1 }}>
              ...
              <input
                ...
                onChange={e => handleChange({ target: { name: "displayPrice", value: e.target.value } } as any)}
                disabled={loading} min="0" step="0.01"
              />
              {formData.charges && formData.displayPrice && (
                <div style={{ fontSize: 11, color: "#16A34A", marginTop: 4, fontWeight: 600 }}>
                  ✅ Customer sees ₹{Number(formData.displayPrice).toLocaleString()} (base: ₹{Number(formData.charges).toLocaleString()})
                </div>
              )}
              <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 4 }}>
                Marked-up price shown to customers
              </div>
            </div>
          </div>
