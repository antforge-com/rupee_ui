 ...
 */
export const getAllActiveQuestions = async (): Promise<BackendQuestion[]> => {
  try {
...
export const getQuestionsBySkills = async (
  ...
  _skillIds: number[]
): Promise<BackendQuestion[]> => getAllActiveQuestions();
...
export const submitAnswers = async (
  ...
  await apiFetch("/answers", {
    method: "POST",
    body: JSON.stringify({ bookingId, consultantId: consultantId ?? null, answers }),
  });
};
...
/**
 * GET /api/users/:userId/bookings/:bookingId/answers
...
export const getAnswersForBooking = async (
  ...
  try {
    const data = await apiFetch(`/users/${userId}/bookings/${bookingId}/answers`);
    return Array.isArray(data) ? data : extractArray(data);
  } catch (err: any) {
    console.warn(`⚠️ getAnswersForBooking(user=${userId}, booking=${bookingId}) failed:`, err?.message);
    return [];
  }
};
