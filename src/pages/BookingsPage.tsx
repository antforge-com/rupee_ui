...
export default function BookingsPage({ isAdmin = false }: Props) {
  ...
  const handleStatusChange = async (bookingId: number, newStatus: string) => {
    ...
    try {
      ...
      showMsg(`Booking #${bookingId} → ${newStatus}`);
    } catch (e: any) {
  ...
  const mapRaw = async (raw: any[]): Promise<Booking[]> => {
    ...
    const mapped: Booking[] = raw.map((b: any) => {
      ...
      const date = ts.slotDate || b.slotDate || b.bookingDate || b.date || "";
          ...
          {filtered.map((b, idx) => {
            ...
            return (
              <div key={b.id} style={{
                ...
                gap: 16, flexWrap: "wrap", boxShadow: "0 1px 4px rgba(0,0,0,0.05)",
              }}>
                <div style={{ width: 46, height: 46, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg,#2563EB,#1D4ED8)", color: "#fff", fontWeight: 700, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {b.userInitial}
                </div>
                <div style={{ flex: 1, minWidth: 180 }}>
                  ...
                  <div style={{ fontSize: 13, color: "#64748B", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {b.time && <span style={{ background: "#EFF6FF", color: "#2563EB", fontWeight: 600, padding: "2px 10px", borderRadius: 20, fontSize: 12 }}>{b.time}</span>}
                    {b.meetingMode && <span>{b.meetingMode === "ONLINE" ? "💻 Online" : "🏢 Offline"}</span>}
                    {b.amount > 0 && <span style={{ color: "#16A34A", fontWeight: 600 }}>₹{b.amount.toLocaleString()}</span>}
                  </div>
                </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 24, flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 0}
              style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: currentPage === 0 ? "not-allowed" : "pointer", border: "1.5px solid #E2E8F0", background: currentPage === 0 ? "#F8FAFC" : "#fff", color: currentPage === 0 ? "#CBD5E1" : "#2563EB" }}>← Prev</button>
            {pageNums().map((pg, i) =>
              pg === "…" ? (
                ...
                <button key={pg} onClick={() => goToPage(pg as number)}
                  style={{ width: 36, height: 36, borderRadius: 8, fontSize: 13, fontWeight: pg === currentPage ? 800 : 600, cursor: "pointer", border: pg === currentPage ? "2px solid #2563EB" : pageCache[pg as number] ? "1.5px solid #BFDBFE" : "1.5px solid #E2E8F0", background: pg === currentPage ? "#2563EB" : pageCache[pg as number] ? "#EFF6FF" : "#fff", color: pg === currentPage ? "#fff" : pageCache[pg as number] ? "#2563EB" : "#374151" }}>
                  {(pg as number) + 1}
                </button>
              )
            )}
            <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage >= totalPages - 1}
              style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: currentPage >= totalPages - 1 ? "not-allowed" : "pointer", border: "1.5px solid #E2E8F0", background: currentPage >= totalPages - 1 ? "#F8FAFC" : "#fff", color: currentPage >= totalPages - 1 ? "#CBD5E1" : "#2563EB" }}>Next →</button>
          </div>
        </div>
      )}
      <style>{`@keyframes mtmPulse { 0% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(37,99,235,0.20)); opacity: 0.0; } 20% { opacity: 0.6; } 50% { transform: scale(1.10); filter: blur(0px) drop-shadow(0 0 22px rgba(37,99,235,0.65)); opacity: 1.0; } 80% { opacity: 0.6; } 100% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(37,99,235,0.20)); opacity: 0.0; } } @keyframes spin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
}
