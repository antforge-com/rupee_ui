...
export const ConsultantNotificationMonitor: React.FC<ConsultantNotificationMonitorProps> = ({
  ...
  const userNameCache = useRef<Map<number, string>>(new Map());  // userId → resolved name
  ...
  const resolveUserName = useCallback(async (userId: number | undefined | null): Promise<string> => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const name = u.name || u.fullName || u.username || u.email?.split("@")[0] || `User #${userId}`;
  ...
  const pollTickets = useCallback(async () => {
    ...
    try {
      ...
      tickets.forEach((t: any) => {
        ...
        const title = t.title || t.description || `Ticket #${t.id}`;

        if (prevStatus === undefined) {
          // First time seeing — check if it's new/open
          const notifId = `ticket_new_${id}`;
          if (!importedIds.current.has(notifId) && ["NEW", "OPEN", "IN_PROGRESS"].includes(st)) {
            importedIds.current.add(notifId);
            newNotifs.push({ id: notifId, type: "warning", title: `Ticket Assigned — #${t.id}`, message: `"${title}" has been assigned to you. Priority: ${t.priority || "MEDIUM"}.`, timestamp: t.createdAt || new Date().toISOString(), read: false, ticketId: t.id });
          }
        } else if (prevStatus !== st) {
          // Status changed
          const notifId = `ticket_${id}_${st}_${Date.now()}`;
          if (st === "RESOLVED") {
            newNotifs.push({ id: notifId, type: "success", title: `Ticket Resolved — #${t.id}`, message: `"${title}" has been marked as resolved.`, timestamp: new Date().toISOString(), read: false, ticketId: t.id });
          } else if (st === "ESCALATED") {
            newNotifs.push({ id: notifId, type: "error", title: `Ticket Escalated — #${t.id}`, message: `"${title}" has been escalated. Immediate action required.`, timestamp: new Date().toISOString(), read: false, ticketId: t.id });
          } else if (st === "CLOSED") {
            newNotifs.push({ id: notifId, type: "info", title: `Ticket Closed — #${t.id}`, message: `"${title}" has been closed.`, timestamp: new Date().toISOString(), read: false, ticketId: t.id });
          }
        }
  ...
  useEffect(() => {
    ...
    // Initial load — populate seen maps without generating toasts
    pollBookings();
    pollTickets();
...
// ─────────────────────────────────────────────────────────────────────────────
// The backend OpenAPI spec provided does NOT define any
//   POST /api/notifications/email/*
// endpoints.
//
// To keep the app aligned with the spec (and avoid 404s), this helper is a no-op.
// In-app notifications still work via localStorage + /api/notifications.
// ─────────────────────────────────────────────────────────────────────────────
