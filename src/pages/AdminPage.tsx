...
import {
  ...
  // createAdminQuestion — now called via apiFetch directly in QuestionsManagementPanel
  createTicket,
  createTicketCategory,
  debugToken,
  // deleteAdminQuestion — now called via apiFetch directly in QuestionsManagementPanel
  deleteAdvisor,
  deleteTicket,
  escalateTicket,
  ...
  // updateAdminQuestion — now called via apiFetch directly in QuestionsManagementPanel
  createSkill,
  deleteSkill,
  getAllSkills,
  updateSkill,
...
// ─────────────────────────────────────────────────────────────────────────────
// QUESTIONS & SKILLS MANAGEMENT PANEL
// Backend: SkillMaster @ /api/skills (GET, POST, PUT /:id, DELETE /:id)
//          QuestionController @ /api/questions (GET ?skillIds=..., POST, PUT /:id, DELETE /:id)
// Flow: Admin creates Skills (categories) → creates Questions per skill
//       → Skills show as categories in user onboarding → Questions shown per selected skill
// ─────────────────────────────────────────────────────────────────────────────
const QuestionsManagementPanel: React.FC = () => {
  ...
  const [qForm, setQForm] = React.useState<{ skillId: string; text: string }>({ skillId: "", text: "" });
  const [filterSkill, setFilterSkill] = React.useState<number | "All">("All");
  ...
  const handleSaveSkill = async () => {
    ...
    try {
      ...
      if (editingSkill?.id) {
        ...
        showToast("Skill created! Switch to Questions tab to add questions for it.");
      }
      await load();
      setShowSkillForm(false);
    } catch (e: any) {
      // Show user-friendly message when /api/skills returns 500
      const msg = String(e?.message || "");
      if (msg.includes("500") || msg.includes("Internal Server")) {
        showToast("Server error — the /api/skills endpoint may not be configured. Check your backend.");
      } else {
  ...
  const handleDeleteSkill = async (id: number) => {
    if (!window.confirm("Delete this skill? All questions linked to it will also be deleted.")) return;
    setDeletingSkill(id);
  ...
  // ── Question CRUD ──
  const openNewQ = () => { setQForm({ skillId: skills[0]?.id?.toString() || "", text: "" }); setEditingQ(null); setShowQForm(true); };
  const openEditQ = (q: QItem) => { setQForm({ skillId: String(q.skillId), text: q.text }); setEditingQ(q); setShowQForm(true); };

  const handleSaveQ = async () => {
    if (!qForm.skillId || !qForm.text.trim()) { showToast("Please select a skill and enter the question."); return; }
    setSavingQ(true);
    try {
      const payload = { skillId: Number(qForm.skillId), text: qForm.text.trim() };
    ...
    } catch (e: any) { showToast(e?.message || "Failed."); } finally { setSavingQ(false); }
  };
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748B" }}>Step 1: Create skills (categories) → Step 2: Add questions per skill</p>
        </div>
        <button onClick={panelTab === "skills" ? openNewSkill : openNewQ}
          ...
          {panelTab === "skills" ? "New Skill" : "New Question"}
        </button>
      </div>
      ...
      <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 12, padding: "12px 16px", marginBottom: 20, fontSize: 13, color: "#1E40AF" }}>
        <strong>How it works:</strong> Skills = categories shown to users during onboarding (e.g. "Tax Planning", "Investment").
        Questions are linked to each skill and shown when the user selects that category.
        Matched consultants are then shown based on their skill tags.
      </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60 }}><div style={{ width: 28, height: 28, border: "3px solid #E2E8F0", borderTopColor: "#2563EB", borderRadius: "50%", animation: "spin 0.7s linear infinite", margin: "0 auto 12px" }} />Loading…</div>
      ) : panelTab === "skills" ? (
        // ── SKILLS TAB ──
        <div>
          {showSkillForm && (
            <div style={{ background: "#F8FAFC", border: "1.5px solid #BFDBFE", borderRadius: 16, padding: 24, marginBottom: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#0F172A", marginBottom: 16 }}>{editingSkill ? "Edit Skill" : "Create New Skill (Category)"}</div>
          ...
          {skills.length === 0 && !showSkillForm ? (
            <div style={{ textAlign: "center", padding: "60px 20px", background: "#F8FAFC", borderRadius: 16, color: "#94A3B8" }}>
              ...
              <p style={{ fontSize: 13, color: "#94A3B8", margin: "0 0 16px" }}>Create your first skill to define the categories users can select during onboarding</p>
              ...
              {skills.map(skill => (
                <div key={skill.id} style={{ background: "#fff", border: "1px solid #F1F5F9", borderLeft: "4px solid #7C3AED", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", gap: 16 }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 4 }}>{questions.filter(q => q.skillId === skill.id).length} question{questions.filter(q => q.skillId === skill.id).length !== 1 ? "s" : ""}</div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => { setPanelTab("questions"); setFilterSkill(skill.id); }} style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #DDD6FE", background: "#F5F3FF", color: "#7C3AED", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>View Questions</button>
        ...
        <div>
          {showQForm && (
            <div style={{ background: "#F8FAFC", border: "1.5px solid #BFDBFE", borderRadius: 16, padding: 24, marginBottom: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#0F172A", marginBottom: 16 }}>{editingQ ? "Edit Question" : "Create New Question"}</div>
              {skills.length === 0 && <div style={{ background: "#FEF9C3", border: "1px solid #FCD34D", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: "#92400E" }}>Please create skills first before adding questions.</div>}
              <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 14, marginBottom: 14 }}>
                <div>
                  <label style={lbl}>Skill *</label>
                  <select value={qForm.skillId} onChange={e => setQForm(f => ({ ...f, skillId: e.target.value }))} style={{ ...inp, cursor: "pointer" }}>
                    <option value="">Select skill…</option>
                    {skills.map(s => <option key={s.id} value={s.id}>{s.name || s.title}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lbl}>Question Text *</label>
                  <input value={qForm.text} onChange={e => setQForm(f => ({ ...f, text: e.target.value }))} placeholder="e.g. What is your primary investment goal?" style={inp} />
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                <button onClick={() => setShowQForm(false)} style={{ padding: "9px 20px", borderRadius: 9, border: "1.5px solid #E2E8F0", background: "#fff", color: "#64748B", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                <button onClick={handleSaveQ} disabled={savingQ} style={{ padding: "9px 22px", borderRadius: 9, border: "none", background: savingQ ? "#93C5FD" : "#2563EB", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{savingQ ? "Saving…" : editingQ ? "Update" : "Create"}</button>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            <button onClick={() => setFilterSkill("All")} style={{ padding: "6px 16px", borderRadius: 20, border: `1.5px solid ${filterSkill === "All" ? "#2563EB" : "#E2E8F0"}`, background: filterSkill === "All" ? "#EFF6FF" : "#fff", color: filterSkill === "All" ? "#2563EB" : "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>All ({questions.length})</button>
            {skills.map(s => { const cnt = questions.filter(q => q.skillId === s.id).length; return (
              <button key={s.id} onClick={() => setFilterSkill(s.id)} style={{ padding: "6px 16px", borderRadius: 20, border: `1.5px solid ${filterSkill === s.id ? "#7C3AED" : "#E2E8F0"}`, background: filterSkill === s.id ? "#F5F3FF" : "#fff", color: filterSkill === s.id ? "#7C3AED" : "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                {s.name} ({cnt})
              </button>
            );})}
          </div>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", background: "#F8FAFC", borderRadius: 16, color: "#94A3B8" }}>
              ...
              <div style={{ fontWeight: 600, color: "#64748B", marginBottom: 8 }}>No questions found</div>
              <p style={{ fontSize: 13, color: "#94A3B8", margin: "0 0 16px" }}>{skills.length === 0 ? "Create skills first, then add questions." : "Add your first question for this skill."}</p>
              <button onClick={openNewQ} style={{ padding: "10px 22px", background: "#2563EB", color: "#fff", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Create First Question</button>
            </div>
          ) : (
            <div style={{ background: "#fff", border: "1px solid #F1F5F9", borderRadius: 16, overflow: "hidden" }}>
              <div style={{ padding: "10px 20px", background: "#F8FAFC", borderBottom: "1px solid #F1F5F9", display: "grid", gridTemplateColumns: "150px 1fr 100px", fontSize: 10, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                <div>Skill</div><div>Question</div><div style={{ textAlign: "right" }}>Actions</div>
              </div>
              {filtered.map((q, idx) => (
                <div key={q.id} style={{ display: "grid", gridTemplateColumns: "150px 1fr 100px", padding: "14px 20px", borderBottom: idx < filtered.length - 1 ? "1px solid #F8FAFC" : "none", alignItems: "center" }}
                  onMouseEnter={e => (e.currentTarget.style.background = "#FAFBFF")}
                  onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                  <div><span style={{ fontSize: 11, fontWeight: 700, color: "#7C3AED", background: "#F5F3FF", border: "1px solid #DDD6FE", borderRadius: 6, padding: "3px 9px" }}>{q.skillName}</span></div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#0F172A" }}>{q.text}</div>
                    {q.updatedAt && <div style={{ fontSize: 10, color: "#94A3B8", marginTop: 2 }}>{new Date(q.updatedAt).toLocaleDateString("en-IN")}</div>}
                  </div>
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                    <button onClick={() => openEditQ(q)} style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #BFDBFE", background: "#EFF6FF", color: "#2563EB", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Edit</button>
                    <button onClick={() => q.id && handleDeleteQ(q.id)} disabled={deletingQ === q.id}
                      style={{ padding: "5px 10px", borderRadius: 7, border: "1px solid #FECACA", background: "#FEF2F2", color: "#DC2626", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", opacity: deletingQ === q.id ? 0.6 : 1 }}>
