  ...
  const handleSendEmailOtp = async () => {
    ...
    try {
      ...
      if (msg.includes("already") || msg.includes("registered") || msg.includes("exist") || msg.includes("400") || msg.includes("duplicate")) {
        ...
        setErrors(x => ({ ...x, email: "⚠️ This email is already registered. Please log in instead." }));
          ...
          <div className="auth-input-group">
            ...
            {mobileNumber.replace(/\D/g,"").length > 0 && mobileNumber.replace(/\D/g,"").length < 10 && !errors.mobileNumber && (
              <div style={{ fontSize: 11, color: "#D97706", marginTop: 4, fontWeight: 600 }}>
                ⚠ Enter {10 - mobileNumber.replace(/\D/g,"").length} more digit{10 - mobileNumber.replace(/\D/g,"").length !== 1 ? "s" : ""}
              </div>
            )}
            {mobileNumber.replace(/\D/g,"").length === 10 && !errors.mobileNumber && (
              <div style={{ fontSize: 11, color: "#16A34A", marginTop: 4, fontWeight: 600 }}>✓ Valid mobile number</div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {plansLoading ? (
              <div style={{ textAlign: 'center', padding: '40px 20px', background: '#fff', borderRadius: 16 }}>
                <img src={logoImg} alt="Meet The Masters"
                  style={{ width: 48, height: 'auto', display: 'block', margin: '0 auto',
                           animation: 'mtmPulse 1.8s ease-in-out infinite' }} />
                <style>{`@keyframes clockSpin { 0%{transform:rotate(0deg)} 100%{transform:rotate(360deg)} } @keyframes mtmPulse { 0% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(37,99,235,0.20)); opacity: 0.0; } 20% { opacity: 0.6; } 50% { transform: scale(1.10); filter: blur(0px) drop-shadow(0 0 22px rgba(37,99,235,0.65)); opacity: 1.0; } 80% { opacity: 0.6; } 100% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(37,99,235,0.20)); opacity: 0.0; } }`}</style>
              </div>
            ) : plans.length === 0 ? (
                ...
                return (
                  <div key={plan.id}
                    ...
                    style={{
                      ...
                      boxShadow: isSelected ? `0 4px 20px ${free ? 'rgba(22,163,74,0.15)' : 'rgba(37,99,235,0.15)'}` : '0 1px 4px rgba(0,0,0,0.04)',
                    }}>
