...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    const alreadyRequiresPwChange = !userAlreadyChangedPw && localStorage.getItem("fin_requires_pw_change") === "true";
    ...
    (async () => {
      try {
        ...
        if (permAlreadyDone) {
          ...
          // User hasn't permanently completed pw change yet — check backend flag
                    ...
                    >
                      ...
                      <div style={{ padding: "14px 18px 18px", borderTop: "1px solid #F1F5F9", marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                        <div style={{ flex: 1 }}>
                          ...
                          {feeLabel && (
                            <div style={{ fontSize: 10, color: "#64748B", marginTop: 1 }}>{feeLabel}</div>
                          )}
                          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 32px 80px rgba(15,23,42,0.3)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 24 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                {[{ icon: "📍", label: "Location", value: profileConsultant.location }, { icon: "🌐", label: "Languages", value: profileConsultant.languages || "English" }, { icon: "📞", label: "Contact", value: profileConsultant.phone || "On request" }, { icon: "💰", label: "Session Fee", value: `₹${calcTotal(profileConsultant.fee).total.toLocaleString()}` }].map(item => (
          ...
          <div className="up-modal" onClick={e => e.stopPropagation()} style={{ padding: 0, overflow: "hidden" }}>
            <div style={{ background: "linear-gradient(135deg,#1E3A5F 0%,#2563EB 100%)", padding: "20px 24px 18px", position: "relative", flexShrink: 0 }}>
              ...
              <p style={{ fontSize: 13, color: "#BFDBFE", margin: "4px 0 0" }}>
                ...
                {calcTotal(selectedConsultant.fee).label && (
                  <span style={{ fontSize: 11, marginLeft: 6, opacity: 0.75 }}>
                    (₹{selectedConsultant.fee.toLocaleString()} base {calcTotal(selectedConsultant.fee).label})
                  </span>
                )}
              </p>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 24, width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 32px 80px rgba(15,23,42,0.4)" }}>
            ...
            <div style={{ background: "linear-gradient(135deg,#1E3A5F,#2563EB)", padding: "24px 24px 20px", position: "sticky", top: 0, zIndex: 1, borderRadius: "24px 24px 0 0" }}>
              {firstLoginStep !== "intro" && firstLoginStep !== "done" && (
                <button onClick={() => {
                  if (firstLoginStep === "details") setFirstLoginStep("intro");  // reset after completion
              ...
              {firstLoginStep === "intro" && (
                <div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
                    {[
                      { icon: "🎯", title: "Personalised Matching", desc: "Get consultants matched to your exact needs." },
                      { icon: "⚡", title: "Quick Setup", desc: "4 simple steps — takes under 3 minutes." },
                      { icon: "🔒", title: "Private & Secure", desc: "Your answers are used only for recommendations." },
                      { icon: "✏️", title: "Always Editable", desc: "Change your preferences anytime from Settings." },
                    ].map((item, i) => (
              ...
              {firstLoginStep === "categories" && (
                <div>
                  ...
                  {dynamicSkillCategories.length > 0 && (() => {
                    ...
                    const catIcon: Record<string, string> = {
                      "Tax": "📄", "Tax Planning": "📄", "Tax Filing": "🗂️", "Tax planner": "🗒️", "International Tax": "🌐",
                      "GST": "📄", "TDS": "📄", "Income Tax": "📄",
                      "Finance": "💵", "Wealth": "💰", "Wealth Management": "💰", "Financial Planning": "📐",
                      "Investment": "📈", "Investments": "📈", "Equity": "📈", "SIP": "📈", "Mutual Fund": "📈",
                      "Insurance": "🛡️", "Life Insurance": "🛡️", "Health Insurance": "🛡️",
                      "Retirement": "🏖️", "Retirement Planning": "🏖️",
                    ...
                    return (
                      <div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12, maxHeight: 300, overflowY: "auto", paddingRight: 4 }}>
                          {dynamicSkillCategories.map(cat => {
                            ...
                            return (
                              ...
                              }} style={{ padding: "12px 14px", borderRadius: 12, textAlign: "left", border: `2px solid ${isSelected ? "#2563EB" : "#E2E8F0"}`, background: isSelected ? "#EFF6FF" : "#fff", color: isSelected ? "#2563EB" : "#374151", fontSize: 12, fontWeight: isSelected ? 700 : 500, cursor: "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        {selected.length > 0 && consultants.length > 0 && (() => {
                          ...
                          return <div style={{ background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 10, padding: "8px 14px", marginBottom: 12, fontSize: 12, color: "#166534", fontWeight: 600 }}>🎯 {matchCount} consultant{matchCount !== 1 ? "s" : ""} match your selection</div>;
                        })()}
                      </div>
                    );
                  })()}
                  <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                    ...
                    <button onClick={async () => {
                      const selected = (firstLoginAnswers["interestedCategories"] || "").split(",").map(s => s.trim()).filter(Boolean);
                      if (selected.length === 0) { setFirstLoginStep("terms"); return; }
                      // Fetch backend questions for selected categories
                      await fetchSkillQuestions(selected);
                      // skillQuestions state won't update synchronously — go to skill-questions always if categories selected
              ...
              {firstLoginStep === "skill-questions" && (
                <div>
                  <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#1E40AF" }}>
                    Based on your selected categories. These answers help consultants prepare for your session.
                  </div>
                    ...
                    <>
                      {skillQuestions.map((q, i) => (
                        <div key={q.id} style={{ marginBottom: 18 }}>
                          ...
                          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 7 }}>{i + 1}. {q.text}</label>
                          <textarea
                            value={skillQAnswers[q.id] || ""}
                            onChange={e => setSkillQAnswers(prev => ({ ...prev, [q.id]: e.target.value }))}
                            placeholder="Your answer…"
                            rows={2}
