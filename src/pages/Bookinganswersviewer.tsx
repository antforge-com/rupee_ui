...
import { BookingAnswer, PostBookingAnswers, getBookingAnswers } from "./Postbookingquestionnaire";
...
const normaliseAnswers = (data: any): PostBookingAnswers | null => {
  if (!data) return null;
  ...
  if (Array.isArray(data.answers)) return data as PostBookingAnswers;
...
const fetchAnswersFromBackend = async (bookingId: number): Promise<PostBookingAnswers | null> => {
  try {
    // Primary endpoint: GET /api/booking-answers/booking/{bookingId}
    const res = await fetch(`${BASE_URL}/booking-answers/booking/${bookingId}`, {
      headers: getAuthHeaders(),
    });
    if (res.ok) {
      const data = await res.json();
      return normaliseAnswers(data);
    }
    // Fallback: GET /api/booking-answers?bookingId={bookingId}
    const res2 = await fetch(`${BASE_URL}/booking-answers?bookingId=${bookingId}`, {
      headers: getAuthHeaders(),
    });
    if (res2.ok) {
      const data2 = await res2.json();
      return normaliseAnswers(Array.isArray(data2) ? data2 : data2?.content ?? data2);
    }
    return null;
  } catch {
    return null;
...
export const BookingAnswersButton: React.FC<{ bookingId: number; clientName: string }> = ({ bookingId, clientName }) => {
  const [open, setOpen] = useState(false);
  const [hasAnswers, setHasAnswers] = useState(false);

  useEffect(() => {
    // Quick check: localStorage first (instant), then backend
    if (getBookingAnswers(bookingId)) { setHasAnswers(true); return; }
    fetch(`${BASE_URL}/booking-answers/booking/${bookingId}`, {
      headers: getAuthHeaders(),
    })
      .then(r => { if (r.ok) setHasAnswers(true); })
      .catch(() => {});
  }, [bookingId]);
  ...
  return (
    <>
      <button
        ...
        style={{ padding: "7px 14px", borderRadius: 8, border: "1.5px solid #BFDBFE", background: "#EFF6FF", color: "#2563EB", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", fontFamily: "inherit" }}
      >
        <IconFileText />
        View Client Profile
      </button>
      {open && <BookingAnswersViewer bookingId={bookingId} clientName={clientName} onClose={() => setOpen(false)} />}
    </>
  );
};

// ─── Full viewer modal ────────────────────────────────────────────────────────
export const BookingAnswersViewer: React.FC<Props> = ({ bookingId, clientName, onClose }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      // 1️⃣ Try backend
      let result = await fetchAnswersFromBackend(bookingId);
      // 2️⃣ Fall back to localStorage cache
      if (!result) result = getBookingAnswers(bookingId) as PostBookingAnswers | null;
      setData(result);
      setLoading(false);
    };
    load();
  ...
  const handlePrint = () => {
    ...
    const html = `
      <html>
      <head>
        <title>Client Profile - ${clientName}</title>
        <style>
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .tag { display: inline-block; background: #EFF6FF; color: #2563EB; border: 1px solid #BFDBFE; border-radius: 20px; padding: 3px 12px; font-size: 12px; font-weight: 600; margin: 2px; }
      ...
      <body>
        <div class="header">
          ...
          <p>${clientName} | Booking #${bookingId} | ${data.dayLabel || ""} ${data.slotLabel || ""}</p>
  ...
  if (loading) {
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 4000, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
    ...
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 4000, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
        <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: 32, maxWidth: 400, width: "100%", textAlign: "center", boxShadow: "0 24px 60px rgba(0,0,0,0.3)" }}>
          ...
          <button onClick={onClose} style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: "#2563EB", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Close</button>
        </div>
      </div>
    );
  }
  ...
  // ── Full viewer ──
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 4000, background: "rgba(15,23,42,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16, backdropFilter: "blur(6px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 600, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(15,23,42,0.4)", overflow: "hidden" }}>
        ...
        {/* PDF Header */}
        <div style={{ background: "linear-gradient(135deg,#1E3A5F,#2563EB)", padding: "22px 24px", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#93C5FD", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 4 }}>
                Pre-Session Client Profile
              </div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 4 }}>{clientName}</div>
              <div style={{ fontSize: 13, color: "#BFDBFE" }}>
                Booking #{bookingId}&nbsp;|&nbsp;{data.dayLabel}&nbsp;|&nbsp;{data.slotLabel}
              </div>
              ...
              <div key={a.questionId || idx} style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 12, padding: "14px 18px", marginBottom: 12 }}>
                ...
                {isMulti ? (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {values.map(v => (
                      <span key={v} style={{ background: "#EFF6FF", color: "#2563EB", border: "1px solid #BFDBFE", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 600 }}>{v}</span>
                    ))}
                  </div>
                ) : (
