  ...
  ({ icon, title, subtitle }) => (
    <div style={{ marginBottom: 20 }}>
      <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#0F172A", display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 34, height: 34, borderRadius: 10, background: "#EFF6FF" }}>{icon}</span>
        {title}
      </h3>
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 20 }}>
        <SectionHeader icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>} title="Ticket Volume" subtitle="Track created vs resolved tickets over time" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 12, marginBottom: 24 }}>
        <StatCard label="Total Created" value={totalCreated} color="#2563EB" bg="#EFF6FF" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>} />
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <SectionHeader icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>} title="Agent Performance" subtitle={mode === "consultant" ? "Your ticket handling performance" : "Compare performance across all consultants"} />
      ...
      {ratings.length === 0 ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 12, marginBottom: 24 }}>
            ...
            <StatCard label="Total Reviews" value={ratings.length} color="#2563EB" bg="#EFF6FF" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>} />
...
const ResponseTimeModule: React.FC<{
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
      <SectionHeader icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} title="Response & Resolution Time" subtitle="How quickly tickets are handled and resolved" />

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 12, marginBottom: 24 }}>
        <StatCard label="Avg Response Time" value={avgResponse > 0 ? fmtHours(avgResponse) : "—"} color="#2563EB" bg="#EFF6FF" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>} />
...
const BookingAnalyticsModule: React.FC<{ bookings: any[] }> = ({ bookings }) => {
  ...
  const chartData = useMemo(() => {
    ...
    bookings.forEach(b => {
      // Exhaustively try every possible date field name the backend might use
      const rawTime =
        b.time || b.slotDate || b.bookingDate || b.date || b.scheduledDate ||
        b.appointmentDate || b.createdAt || b.created_at || "";
      if (!rawTime) return;
      // Strip time portion — handle "2026-03-06 • 09:00 - 10:00", ISO, plain date
      const dateStr = rawTime.includes(" • ")
        ? rawTime.split(" • ")[0].trim()
        : rawTime.split("T")[0].replace(" ", "");
      ...
      let label = "";
      let isoKey = dateStr;
      if (period === "daily") { label = dayLabel(date); isoKey = dateStr; }
      ...
      if (st === "COMPLETED" || st === "SUCCESS" || st === "CONFIRMED") {
        dataMap[label].revenue += (b.amount || b.charges || b.fee || b.totalAmount || 0);
      }
    });
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E2E8F0", padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 12, marginBottom: 24 }}>
        <StatCard label="Total Bookings" value={totalBookings} color="#2563EB" bg="#EFF6FF" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} />
        <StatCard label="Total Revenue" value={`₹${totalRevenue.toLocaleString()}`} color="#059669" bg="#F0FDF4" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>} />
        ...
        <StatCard label="Avg Ticket Size" value={`₹${avgTicketSize.toLocaleString()}`} color="#D97706" bg="#FFFBEB" icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>} />
      </div>
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!bookingsProp || bookingsProp.length === 0) {
        try {
          const ep = mode === "consultant" && consultantId
            ? `/bookings/consultant/${consultantId}` : "/bookings";
          const data = await apiFetch(ep);
          const arr = extractArr(data);
          if (arr.length > 0) {
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? "linear-gradient(135deg,#1E3A5F,#2563EB)" : "#fff",
