...
// ─────────────────────────────────────────────────────────────────────────────
// PasswordInput — reusable input with eye toggle
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="auth-page">
      <button
        ...
        className="glass-btn"
        style={{ position: "absolute", top: 24, left: 24, width: 42, height: 42 }}
        title="Back to Login"
      >
        <ArrowLeft size={20} />
      </button>
      ...
      <div className="auth-card animate-fade-up">
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: "#0F172A", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
            {step === "done" ? <><CheckCircle className="icon-sm" style={{ color: "var(--color-success)" }} /> Password Reset!</> : "Reset Your Password"}
          </div>
        ...
        {step !== "done" && (
          <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
            {["email", "otp", "newpass"].map((s, i) => (
              <div key={s} style={{ height: 4, flex: 1, borderRadius: 4, background: stepIndex >= i ? "var(--color-primary)" : "var(--border-color)", transition: "background 0.3s" }} />
            ))}
          </div>
        )}
        ...
        {step === "otp" && (
          <div className="auth-input-group">
            ...
            <input
              ...
              autoFocus
              className="input-base"
              style={{ fontSize: 24, letterSpacing: "0.5em", textAlign: "center" }}
            />
            ...
            {error && <div className="error-banner" style={{ marginTop: 12 }}><AlertTriangle size={16} /> {error}</div>}
            <button onClick={handleOtpContinue} disabled={otp.length !== 6} className="btn-primary" style={{ width: "100%" }}>
              Continue
            </button>
          </div>
        )}
        ...
        {step === "newpass" && (
          <div className="auth-input-group">
            ...
            <PasswordInput
              ...
              hasError={!!(confirmPass && confirmPass !== newPassword)}
            />
...
export default function LoginPage() {
  ...
  const classifyError = (err: any): { msg: string; type: ErrorType } => {
    ...
    if (
      ...
      return { msg: "Incorrect email or password.", type: "auth" };
    return { msg: err?.message || "Login failed.", type: "auth" };
  };
  ...
  const redirectByRole = (rawRole: string, isFirstLogin = false) => {
    ...
    if (isFirstLogin) localStorage.setItem("fin_first_login", "true");
    ...
    if (role === "USER" || role === "SUBSCRIBER" || role === "SUBSCRIBED" || role === "GUEST") navigate("/user");
      ...
      setApiError(`Role not recognized: "${rawRole || "empty"}".`);
      setErrorType("auth");
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (requiresPasswordChange) localStorage.setItem("fin_first_login", "true");
  ...
  const handleGuestLogin = () => {
    ...
    setTimeout(() => { navigate("/user"); setGuestLoading(false); }, 200);
  };
  ...
  return (
    <div className="auth-page">
      ...
      <button
        onClick={() => navigate("/")}
        className="glass-btn"
        style={{ position: "absolute", top: 24, left: 24, width: 40, height: 40 }}
        title="Back to Home"
      >
        <ArrowLeft size={20} />
      </button>
      ...
      <div className="auth-card animate-fade-up">
        ...
        <p className="auth-tagline">YOUR FINANCIAL ADVISORY PLATFORM</p>
        ...
        <div
          ...
          style={{
            ...
            border: `1.5px solid ${termsAccepted ? "var(--color-success-border)" : termsShake ? "var(--color-danger-border)" : "var(--border-color)"}`,
        ...
        >
          ...
          <label htmlFor="terms-checkbox" style={{ fontSize: 13, color: "var(--text-main)", lineHeight: 1.5, cursor: "pointer" }}>
            ...
            I agree to the{" "}
            <span onClick={e => { e.preventDefault(); setShowTermsModal(true); }} className="auth-link">
              Terms & Conditions
            </span>
            {" "}and{" "}
            <span onClick={e => { e.preventDefault(); setShowTermsModal(true); }} className="auth-link">
        ...
        {apiError && (
          <div className="error-banner">
            {errorType === "server" ? <Settings size={18} /> : errorType === "network" ? <WifiOff size={18} /> : <AlertTriangle size={18} />}
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 800 }}>{errorType === "server" ? "Server Error" : errorType === "network" ? "Connection Error" : "Login Failed"}</div>
        ...
        >
          {loading
            ? <><span className="animate-spin" style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block" }} /> Authenticating...</>
            : "Login to Account"
          }
        </button>
