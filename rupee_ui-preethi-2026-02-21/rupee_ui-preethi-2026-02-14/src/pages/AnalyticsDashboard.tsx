...
const AgentPerformanceModule: React.FC<{
  ...
  const agentStats = useMemo(() => {
    ...
    tickets.forEach(t => {
      const key = t.consultantName || t.agentName || (t.consultantId ? `#${t.consultantId}` : "Unassigned");
      ...
      // response time (createdAt → firstResponseAt or updatedAt if status changed)
  ...
  // In consultant mode, only show that consultant
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#F8FAFC" }}>
                  {["Agent", "Total", "Resolved", "Open", "Rate", "Avg Response", "Avg Resolution"].map(h => (
                    ...
                    <tr key={a.name} style={{ borderTop: "1px solid #F1F5F9", transition: "background 0.1s" }}
                      ...
                      <td style={{ padding: "12px 14px", fontWeight: 700, color: "#0F172A" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 30, height: 30, borderRadius: "50%", background: `${COLORS[i % COLORS.length]}20`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: COLORS[i % COLORS.length] }}>
...
const AnalyticsDashboard: React.FC<Props> = ({
  ...
  return (
    <div>
      ...
      {activeModule === "agents" && <AgentPerformanceModule tickets={tickets} consultants={consultants} mode={mode} consultantId={consultantId} />}
