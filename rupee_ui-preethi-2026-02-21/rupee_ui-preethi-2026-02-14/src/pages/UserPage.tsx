...
// ─────────────────────────────────────────────────────────────────────────────
// LOCAL API HELPERS  (retained for endpoints not in the service layer)
// ─────────────────────────────────────────────────────────────────────────────
const BASE_URL = "/api";
...
// Ticket types — extended to cover both UserPage & UserTickets shapes
...
// ─────────────────────────────────────────────────────────────────────────────
// TICKET CONFIG  (merged from UserPage + UserTickets priorities/statuses)
// ─────────────────────────────────────────────────────────────────────────────
...
// Ordered categories from UserTickets.tsx (richer set)
...
// Stepper steps — extended to cover IN_PROGRESS
const TICKET_STEPS = [
...
const getStepIndex = (status: string) => {
  ...
  // PENDING maps to IN_PROGRESS position; ESCALATED maps to OPEN
  if (idx === -1) {
...
// ─────────────────────────────────────────────────────────────────────────────
// SHARED BADGE  (from UserTickets.tsx)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// CREATE TICKET MODAL  (upgraded from UserTickets.tsx)
// — Better validation (10-char min), URGENT priority, richer category set
// — Routes to createTicket() from ../services/api
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// TICKET DETAIL MODAL  (upgraded from UserTickets.tsx)
// — SLA banner, auto-scroll thread, feedback section, api.ts calls
// ─────────────────────────────────────────────────────────────────────────────
const TicketDetailModal: React.FC<{
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessage("");
      // Auto-advance status NEW → OPEN on first user reply
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1200, backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 20, width: 560, maxWidth: "96vw", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 80px rgba(0,0,0,0.3)", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
        ...
        {/* SLA banner — customer-friendly wording */}
        {sla && (
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
                  ...
                  return (
                    <div key={c.id} style={{ display: "flex", justifyContent: isAgent ? "flex-start" : "flex-end" }}>
                      <div style={{
                        ...
                        background: isAgent ? "linear-gradient(135deg,#1E3A5F,#2563EB)" : "#EFF6FF",
          ...
          {/* Feedback — only when resolved and no feedback yet */}
      ...
      <header className={styles.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button onClick={handleGoToProfile} title="My Profile"
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.08)"; }} onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1)"; }}>
          ...
          <div className={styles.tabPadding}>
            ...
            <div className={styles.ticketFilterRow} style={{ flexWrap: "wrap" }}>
              {STATUS_FILTERS.map(f => {
                ...
                return (
                  <button key={f} onClick={() => setTicketFilter(f as any)}
                    className={`${styles.ticketFilterBtn} ${ticketFilter === f ? styles.ticketFilterBtnActive : ""}`}>
